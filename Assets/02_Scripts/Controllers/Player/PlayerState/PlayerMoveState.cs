        ...
        if (_player._rotDir != Vector3.zero)
        {
            switch (_player._playerCam._cameraMode)
            {
                case Define.CameraMode.QuarterView:
                    ...
                    // 이동 방향
                    _player._moveDir = _player._playerModel.forward * Managers.Stat.MoveSpeed * Time.fixedDeltaTime;
                    break;

                case Define.CameraMode.ZoomView:
                    _player._moveDir = _player._rotDir * Managers.Stat.MoveSpeed * Time.fixedDeltaTime;
                    break ;
            }
        }
        else
